...
D=m1+m2;       %2进制串总位数
t=1;
G=randint(10,D);%初始种群2进制编码
while t<2000   %控制遗传代数
U1=ones(10,m1); %取与a对应的2进制串
for i=1:1:10
  U1(i,:)=G(i,1:m1);
end  
U2=ones(10,m2); %取与b对应的2进制串
for i=1:1:10
  U2(i,:)=G(i,(m1+1):D);
end 
X=ones(10,2);  %a,b区间内的任意取值
W=1;
for i=1:1:10 
 S1=0;
  for j=1:1:m1
...
SP=ones(10,1);           %sum of probility 到该个体时的概率之和 
S4=0;
for i=1:1:10
  S4=S4+evaluate(i,1);
  SP(i,1)=S4;
end  
R=rand(10,1);            %建立10个随机数
G1=ones(10,D);          %定义子一代(刚完成选择，还未变异、交换)
for i=1:1:10
    if R(i,:)<=SP(1,:)  
       G1(i,:)=G(1,:);
     else if R(i,:)<=SP(2,:)
...
P1=0.25;                   %发生交换的概率
R2=rand(10,1);            %产生10个随机数
Count=zeros(10,1);        %选出被选中的个体
c=0;                      %统计被取出的个体数                     
for i=1:1:10
  if R2(i,1)<P1  
    Count(i,1)=1;
...
G=G1;
G1=ones(10,D);                                    %注意已将G1复位
t=t+1;
end
U1=ones(10,m1);                                   %一下与程序开头重复，用于求出最优解
for i=1:1:10
  U1(i,:)=G(i,1:m1);
end  
U2=ones(10,m2);
...
for i=2:1:10
   if evaluate2(i,1)>evaluate2((i-1),1)
       ...
       i=i+1;                                 %判断最优解(最优解为evaluate2(10,1))
   end
end 
for i=1:1:10
